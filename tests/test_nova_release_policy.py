...
def test_required_reviewed_training_seed_overrides_broad_data_exclusion(
    ...
    builtin_dictionary = policy.classify(
        "data/builtin_word_dictionary.json",
        size_bytes=128,
    )
    mutable_dictionary = policy.classify(
    ...
    assert builtin_dictionary.classification is SnapshotClass.INCLUDE
    assert builtin_dictionary.rule == "required_files"
    assert safe_model_defaults.classification is SnapshotClass.INCLUDE
    assert safe_model_defaults.rule == "required_files"
    assert mutable_dictionary.classification is SnapshotClass.EXCLUDE
