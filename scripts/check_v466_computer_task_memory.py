#!/usr/bin/env python3
"""Check v466_computer_task_memory."""
import sys; from pathlib import Path
ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))
from v466_computer_task_memory import store_task
E,P=[], []
def c(cond,msg):
    if cond: P.append(f"  [PASS] {msg}")
    else: E.append(f"  [FAIL] {msg}")
def main():
    print(f"Nova v466_computer_task_memory -- Checker\n")
    c(Path(ROOT/"src"/"v466_computer_task_memory.py").exists(), "src exists")
    r = store_task()
