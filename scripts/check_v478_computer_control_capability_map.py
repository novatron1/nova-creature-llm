#!/usr/bin/env python3
"""Check v478_computer_control_capability_map."""
import sys; from pathlib import Path
ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))
from v478_computer_control_capability_map import capability_map
E,P=[], []
def c(cond,msg):
    if cond: P.append(f"  [PASS] {msg}")
    else: E.append(f"  [FAIL] {msg}")
def main():
    print(f"Nova v478_computer_control_capability_map -- Checker\n")
    c(Path(ROOT/"src"/"v478_computer_control_capability_map.py").exists(), "src exists")
    r = capability_map()
