#!/usr/bin/env python3
"""Check v435_grant_proposal."""
import sys; from pathlib import Path
ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))
from v435_grant_proposal import simulate_grant_proposal
E,P=[], []
def c(cond,msg):
    if cond: P.append(f"  [PASS] {msg}")
    else: E.append(f"  [FAIL] {msg}")
def main():
    print(f"Nova v435_grant_proposal -- Checker\n")
    c(Path(ROOT/"src"/"v435_grant_proposal.py").exists(), "src exists")
