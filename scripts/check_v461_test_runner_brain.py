#!/usr/bin/env python3
"""Check v461_test_runner_brain."""
import sys; from pathlib import Path
ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))
from v461_test_runner_brain import run_tests
E,P=[], []
def c(cond,msg):
    if cond: P.append(f"  [PASS] {msg}")
    else: E.append(f"  [FAIL] {msg}")
def main():
    print(f"Nova v461_test_runner_brain -- Checker\n")
    c(Path(ROOT/"src"/"v461_test_runner_brain.py").exists(), "src exists")
    r = run_tests()
