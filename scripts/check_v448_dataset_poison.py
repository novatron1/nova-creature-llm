#!/usr/bin/env python3
"""Check v448_dataset_poison."""
import sys; from pathlib import Path
ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))
from v448_dataset_poison import simulate_dataset_poison
E,P=[], []
def c(cond,msg):
    if cond: P.append(f"  [PASS] {msg}")
    else: E.append(f"  [FAIL] {msg}")
def main():
    print(f"Nova v448_dataset_poison -- Checker\n")
    c(Path(ROOT/"src"/"v448_dataset_poison.py").exists(), "src exists")
