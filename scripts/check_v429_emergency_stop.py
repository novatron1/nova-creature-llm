#!/usr/bin/env python3
"""Check v429_emergency_stop."""
import sys; from pathlib import Path
ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))
from v429_emergency_stop import simulate_emergency_stop
E,P=[], []
def c(cond,msg):
    if cond: P.append(f"  [PASS] {msg}")
    else: E.append(f"  [FAIL] {msg}")
def main():
    print(f"Nova v429_emergency_stop -- Checker\n")
    c(Path(ROOT/"src"/"v429_emergency_stop.py").exists(), "src exists")
